...
Why: scripts/v8/exp1ab_judge_sensitivity.py only aggregates A/B distributions
...
Outputs:
  ...
  revision/revision_exp_results/exp_2_0_judge/
      Disagreement summary (per-source + per-strategy) and stratification
      input for the annotation step.

Reuses 4 A/B variants from scripts/v8/exp1ab_judge_sensitivity.py verbatim
(we drop "causal_final_answer" — it's a C-vs-NC variant, not an A/B
classifier, and 2.1 only re-labels non-C records).

Usage:
  python revision/scripts/exp_2_0_judge_persistence.py --api-key $ANTHROPIC_API_KEY
  python revision/scripts/exp_2_0_judge_persistence.py --sample 500 --api-key $KEY   # smoke test
"""
...
OUTDIR = ROOT / "revision" / "revision_exp_results" / "exp_2_0_judge"
...
# Four A/B variants from scripts/v8/exp1ab_judge_sensitivity.py.
PROMPTS = {
