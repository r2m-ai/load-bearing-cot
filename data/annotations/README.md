...
The blinded sheets are the original blank materials sent to annotators. The annotators' returned label files are not released; the aggregate numbers they produced are in the `results_*.json` files and in the paper. Load the frames without it if you intend to rerun the exercise, and keep the metadata hidden while you do.
