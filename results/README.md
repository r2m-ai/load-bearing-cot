...
Snapshot contents have been retained during reorganization; experiment identifiers inside them describe historical provenance. Full continuation records, hidden-state arrays and steering generations are not shipped. Generated steering outputs can be placed in `results/steering/` by the relevant workflow; no empty directory is presented as a released result.
