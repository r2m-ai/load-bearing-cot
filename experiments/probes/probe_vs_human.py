...
Usage:
  python experiments/probes/probe_vs_human.py
"""
...
ANNOT = ROOT / "data" / "annotations" / "pilot_n200" / "data_to_label.csv"
META = ROOT / "data" / "annotations" / "pilot_n200" / "_analysis_metadata.csv"
OUTDIR = ROOT / "results" / "probes" / "human_agreement"
...
def train_holdout_probe(layer: int = LAYER) -> dict:
    ...
    # how experiments/probes/retrain_probes.py (the paper's training script) does it.
...
def predict_on_annot_features(probe: dict, layer: int = LAYER) -> dict:
    ...
    if not ANNOT_FEATURES.exists():
        print(f"   (no {ANNOT_FEATURES.name} — non-overlap predictions skipped; "
              "run experiments/probes/extract_human_annotation_features.py on a GPU pod)")
        return {}
