"""Paper figure: base accuracy vs error propagation at subject-level resolution.

Plots every MMLU subject, BBH subtask and GSM8K as its own point, with the
dataset averages for the three model families overlaid, and writes
figures/difficulty_gradient_unified.png.

Needs the regenerated data/processed/expanded_pairs.json (see
docs/paper-reproduction.md); run from the repository root:
    python experiments/figures/plot_difficulty_gradient.py
"""
